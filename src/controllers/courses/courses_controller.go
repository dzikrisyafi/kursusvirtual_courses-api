...
import (
	...
	"github.com/dzikrisyafi/kursusvirtual_courses-api/src/utils/errors"
	"github.com/dzikrisyafi/kursusvirtual_oauth-go/oauth"
	"github.com/dzikrisyafi/kursusvirtual_utils-go/rest_errors"
	"github.com/gin-gonic/gin"
)

func getCourseId(courseIdParam string) (int64, *errors.RestErr) {
	courseID, err := strconv.ParseInt(courseIdParam, 10, 64)
	if err != nil {
		return 0, errors.NewBadRequestError("course id should be a number")
	}
	return courseID, nil
}
...
func Get(c *gin.Context) {
	courseID, err := getCourseId(c.Param("course_id"))
	if err != nil {
		c.JSON(err.Status, err)
		return
	}
...
func Search(c *gin.Context) {
	...
	courses, err := services.CoursesService.SearchCourse(courseName)
	if err != nil {
		c.JSON(err.Status, err)
		return
	}
