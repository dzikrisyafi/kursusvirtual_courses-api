...
func (cohort *Cohort) Save() rest_errors.RestErr {
	stmt, err := courses_db.DbConn().Prepare(queryInsertCohort)
	if err != nil {
...
func (cohort *Cohort) Get() rest_errors.RestErr {
	stmt, err := courses_db.DbConn().Prepare(queryGetCohort)
	if err != nil {
...
func (cohort *Cohort) GetAll() ([]Cohort, rest_errors.RestErr) {
	stmt, err := courses_db.DbConn().Prepare(queryGetAllCohort)
	if err != nil {
...
func (cohort *Cohort) Update() rest_errors.RestErr {
	stmt, err := courses_db.DbConn().Prepare(queryUpdateCohort)
	if err != nil {
...
func (cohort *Cohort) Delete() rest_errors.RestErr {
	stmt, err := courses_db.DbConn().Prepare(queryDeleteCohort)
	if err != nil {
