...
import (
	...
	"github.com/dzikrisyafi/kursusvirtual_courses-api/src/datesources/mysql/courses_db"
...
func (course *Course) Save() *rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to save course", rest_errors.NewError("database error"))
	}
	defer stmt.Close()
...
func (course *Course) Get() *rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to get course by id", rest_errors.NewError("database error"))
	}
	defer stmt.Close()
...
func (course *Course) Update() *rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to update course", rest_errors.NewError("database error"))
	}
	defer stmt.Close()
...
func (course *Course) Delete() *rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to delete course", rest_errors.NewError("database error"))
	}
	defer stmt.Close()
...
func (course *Course) FindCourseByName() ([]Course, *rest_errors.RestErr) {
	...
	if err != nil {
		...
		return nil, rest_errors.NewInternalServerError("error when trying to find course", rest_errors.NewError("database error"))
	}
	defer stmt.Close()
